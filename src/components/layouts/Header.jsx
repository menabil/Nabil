      ...
      >
        <Container>
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-x-10">
              <ul className="hidden md:flex items-end gap-9 list-none mb-0">
                {["Home", "About", "Portfolio", "Contact"].map((item) => (
                  <li key={item}>
                    <SmoothLink href={`#${item.toLowerCase()}`}>
                      <a className="nav-link relative no-underline text-gray-800 text-sm font-medium transition-colors hover:text-primary beforeAfter">
                        {item}
                      </a>
                    </SmoothLink>
                  </li>
              ...
              <SmoothLink href="#contact">
                <a className="hidden md:inline-flex items-center gap-2 px-[22px] py-2.5 rounded-full bg-primary text-white font-semibold text-[13px] transition-all duration-300 border-3 border-primary hover:border-indigo-200 hover:bg-transparent hover:text-primary no-underline">
                  Hire Me
                </a>
              </SmoothLink>
            </div>
            ...
            >
              <span
                className={`w-6 h-0.5 bg-gray-800 rounded transition-all ${isOpen ? "rotate-45 translate-y-2" : ""}`}
              ></span>
              <span
                className={`w-6 h-0.5 bg-gray-800 rounded transition-all ${isOpen ? "opacity-0" : ""}`}
              ></span>
              <span
                className={`w-6 h-0.5 bg-gray-800 rounded transition-all ${isOpen ? "-rotate-45 -translate-y-1.5" : ""}`}
              ></span>
            </div>
          </div>
          {/* Mobile Menu */}
